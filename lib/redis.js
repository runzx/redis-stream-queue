...
class RedisBase {
  ...
  _initRedis(opt) {
    let {
      client, host = '127.0.0.1', port = 6379, password = '', db = 3,
      sKey, gKey, cKey, maxlen
    } = opt
  ...
  async start() {
    ...
    if (this.sKey && this.gKey) {
      const key = await this.client.exists(this.sKey)
      // if (key === 0) 
  ...
  async getMsgByConsumerBlock(cKey = this.cKey, count = 1) {
    ...
    let res = await this.clientBlock.xreadgroup('GROUP', this.gKey, cKey, 'COUNT', count, 'BLOCK', 0, 'STREAMS',
      this.sKey, '>')
   ...
   * @returns {[{name,pending:number,idle}]}
   * idle: 空闲时间 ms,
   * pending: 待处理消息数量
   */
   ...
   * 查询 待处理 列表 PEL list
   * @param {string} cKey
   * @param {number} count 取数量,默认 100
   * @param {string} gKey
   * @param {string} sKey
   * @returns {[{ id, consumer, idle, count }]}
   */
  async getPELList(count = 100, cKey = this.cKey, gKey = this.gKey, sKey = this.sKey) {
    let res = await this.client.xpending(sKey, gKey, '-', '+', count, cKey)
    debug('PEL list: %O', res)

    return res.map(([id, consumer, idle, count]) => ({ id, consumer, idle, count }))
  }
  /**
   * 取 空闲时间>idel的 PEL列表
   * start, count 可组合 分页
   ...
   * @returns 
   */
  async getPELListByIDLE({ idle = 5, count = 100, cKey = this.cKey, gKey = this.gKey, sKey = this.sKey,
    start = '-', end = '+' } = {}) {
    let res = await this.client.xpending(sKey, gKey, 'IDLE', idle * 1000, start, end, count, cKey)
    debug('PEL list: %O', res)
  ...
  delMsgById(id, sKey = this.sKey) {
    return this.client.xdel(sKey, id)
  }
}
