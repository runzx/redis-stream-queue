...
class RedisBase {
   ...
   * @returns {[{id,item}]}
   */
  async getMsgByConsumer(cKey = this.cKey, count = 1, block) {
    let res = block ? await this.client.xreadgroup('GROUP', this.gKey, cKey, 'COUNT', count, 'BLOCK', 0, 'STREAMS', this.sKey, '>')
      : await this.client.xreadgroup('GROUP', this.gKey, cKey, 'COUNT', count, 'STREAMS', this.sKey, '>')
