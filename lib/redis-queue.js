...
class DelayQueue extends RedisQueue {
  ...
  pubExEvent(client = this.client, db = this.exEventDb) {
  ...
  delaySubSubcribe() {
    ...
    if (!stream) {
      this.delayStream = stream = new RedisQueue({ ...this.opt, sKey: this.sKey + ':DELAY_STREAM', maxlen: 10000 })
    }
    if (!stream.subWating) {
    ...
    stream.events.on(stream.sKey, async (msg) => {
      const { id, item } = msg
      if (!id) {
        console.log('delay task events err:', msg)
        return
      }
  ...
  async setDataTTL(id, expiresZxAt) {
    ...
    let res = await this.clientSub.set(key, 1, "EX", ttl)
    if (res !== 'OK')
  ...
  async postDelayMsg(id, msg) {
    ...
    let ack = await this.delayStream.ack(id)
