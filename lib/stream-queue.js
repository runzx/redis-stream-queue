...
class StreamQueue {
  ...
  static async init({
                      sKey = STREAM_KEY, gKey = GROUP_NAME,
...
const defaultOptDelayStream = {
  ...
  cKey: 'mmDelayStreamConsumer', // consumer name
}
...
class DelayQueue {
  ...
  intervalTime = 60 * 60 * 1000 // 5m    5 * 60 * 1000 
  intervalId  // 定时id
  constructor(opt) {
...
class RedisQueue {
  ...
  _initialize({ intervalTime, client, delayOpt }) {
    this.client = client
    delayOpt && (this.delayOpt = delayOpt)
    intervalTime && (this.intervalTime = intervalTime)
    // this.service()
    // this.checkedPending()
  }
  ...
  async subcribe(streamKey, cb) {
    if (!this.subscriptions[streamKey]) {
      this.subscriptions[streamKey] = await StreamQueue.init({
        sKey: streamKey, client: this.client,
        events: this.events,
        // gKey: 'streamDemoGroup',
        // cKey: 'streamDemoConsumer'
      })
  ...
  /**
   * 插入消息入队列
   * @param streamKey 消息队列名
   * @param info 消息
   * @param ttl 延迟执行时间 秒 (>0创建 延迟消息队列)
   * @returns {Promise<void|*>|void|*|Promise<void|*>|void}
   */
  addTask(streamKey, info, ttl) {
    ...
    return this.subscriptions[streamKey].addTask(this.preData(info))
  }

  async addDelayTask(streamKey, info, ttl = 3) {
    if (!ttl && +ttl <= 0) return console.log('delay ttl must > 0s: ', ttl)

    if (!this.delayQueues[streamKey]) await this.preDelay(streamKey, this.delayOpt)

    return this.delayQueues[streamKey].addTask(streamKey, info, ttl,)
  }
  ...
  async preDelay(streamKey, { db, sKey }) {
    this.delayQueues[streamKey] = await DelayQueue.init({
      client: this.client, db, sKey: `${sKey}:${streamKey}`,
    })
    await this.delayQueues[streamKey].start()
  }
  ...
  retryTask(list, pendingList, streamModel) {
    ...
    res.forEach(({ id }) => {
      streamModel.ack(id)  // xack 移出pending表，不再重试
    })
...
module.exports = { StreamQueue, DelayQueue, RedisQueue }